...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if (f_pos > dir_oi->oi_size / OSPFS_DIRENTRY_SIZE) // TODO check?
		{
			r = 1;
			break;
		}
...
allocate_block(void)
{
	...
	{
		// checks if it is free. 1 means that it is free. 
		if (bitvector_test(bitmap, i))
		{
			bitvector_clear(bitmap, i); // mark it as allocated. 
			return i; // return the value of the allocated block.
		}
	}
...
free_block(uint32_t blockno)
{
	...
	// TODO check if this is accurate (or at least that it doesn't affect our code.
	int firstDataBlock = ospfs_super->os_firstinob + ospfs_super->os_ninodes / OSPFS_BLKINODES;
	if (blockno < firstDataBlock)
	{
...
{
	if (b < OPFS_NDIRECT + OSPFS_NINDIRECT)
		return -1;
	return 0;
}
...
direct_index(uint32_t b)
{
	...
	return (b - OSPFS_NDIRECT - OSPFS_NINDIRECT) % OSPFS_NINDIRECT;
	// - OSPFS_NINDIRECT is actually unneccessary, but good for clarity purposes. 
}
...
add_block(ospfs_inode_t *oi)
{
	...
	if ( n < 0 ) // this should never be true. 
		return -EIO;
	...
	if (n < OSPFS_NDIRECT)
	{
		...
		if (block_index == 0) // block index == 0 only when there was no space. 
			return -ENOSPC;
		// zero out new block. 
		memset(ospfs_block(block_index), 0, OSPFS_BLKSIZE);

		storage = oi->oi_direct;
		oi->oi_direct[n] = block_index;
	}
	else if (n < OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		if (oi->oi_indirect == 0)
		{
			...
			allocated[0] = (uint32_t*) ospfs_block(block_index);
			oi->oi_indirect = block_index;
		}

		block_index = allocate_block();
		if (block_index == 0)
		{
			if (allocated[0] != 0) // should always return true. 
			{
		...
		storage = (uint32_t*)ospfs_block(oi->oi_indirect);
	}
	else if (n < OSPFS_MAXFILEBLKS)
	{
		if (oi->oi_indirect2 == 0)
		{
			...
			allocated[0] = (uint32_t*)ospfs_block(block_index);
			oi->oi_indirect2 = block_index;
		}

		// finds the indirect_block that the new block should be in. 
		indirect_block2 = (uint32_t*)ospfs_block(oi->oi_indirect2);
		// if the indirect block has not been allocated yet...
		if (indirect_block2[indir_index(n)] == 0)
		{
			// allocate a new indirect block. 
			block_index = allocate_block();
			if (block_index == 0)
			...
			allocated[1] = (uint32_t*)ospfs_block(block_index);
			indirect_block2[indir_index(n)] = block_index;
		}

		// at this point, we know that indirect, indirect^2 blocks have been made. 
		block_index = allocate_block();

		// TODO double check some of the values in here. 
		if (block_index == 0)
		{
			if (allocated[0] != 0)
			{
			...
			if (allocate[1] != 0)
			{
				free_block(indirect_block2[indir_index(n)]);
				indirect_block2[indir_index(n)] = 0;
			}
			return -ENOSPC;
		}
		memset(ospfs_block(block_index), 0, OSPFS_BLKSIZE);

		storage = (uint32_t*)ospfs_block(indirect_block2[indir_index(n)]);
	}
	else // can't add a block if all blocks already filled. 
	{
		return -ENOSPC;
	}
	storage[direct_index(n)] = block_index; // TODO check if this works correctly. 
											// i just moved this out for efficiency purposes.
											// easy to remove. 
...
remove_block(ospfs_inode_t *oi)
{
	// current number of blocks in file
	uint32_t n = ospfs_size2nblocks(oi->oi_size);
	n -= 1; // TODO check off-by-one error?
	if (n < 0)
		return -EIO;
	if (n < OSPFS_NDIRECT)
	{
		if (oi->oi_direct[n] == 0)
			return -EIO; // weird because it should be allocated. TODO change?
	...
	{
		uint32_t* indirect_block2 = (uint32_t*)ospfs_block(oi->oi_indirect2);
		uint32_t* indirect_block = (uint32_t*)ospfs_block(indirect_block2[indir_index(n)]);

		// free block. 
		free_block(indirect_block[direct_index(n)]);
		indirect_block[direct_index(n)] = 0;

		// free this indirect block if this indirect block is now impty. 
		if (direct_index(n) == 0)
		{
			free_block(indirect_block2[indir_index(n)]);
			indirect_block2[indir_index(n)] = 0;

			// free the indirect^2 block if we removed last indirect block. 
			if (indir_index(n) == 0)
			{
				free_block(oi->oi_indirect2);
				oi->oi_indirect2 = 0;
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	if (r == -ENOSPC)
	{
		while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size))
			remove_block(oi);
		oi->oi_size = old_size; // shouldn't be necessary. 
		return r;
	}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		// checks the offset of the file in the block. 
		int offset = *fpos % OSPFS_BLKSIZE;
		if (count - amount > OSPFS_BLKSIZE - offset) // the file to be read requires more than one block. 
		{
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		int offset = *f_pos % OSPFS_BLKSIZE;
		if (count - amount > OSPFS_BLKSIZE - offset) // the file to be written requires more than one block. 
		{
			n = OSPFS_BLKSIZE - offset;
		}
		else
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	ospfs_direntry_t *entry; // new direntry. 
	int retval;

	uint32_t offset;
	for (offset = 0; offset < dir_oi->oi_size; entry += OSPFS_DIRENTRY_SIZE)
	{
		entry = (ospfs_direntry_t*) ospfs_inode_data(dir_oi, offset);
		if (entry->od_ino == 0) // entry is empty. 
			return entry;
	}
...
{
	// name length check. (symname doesn't exist in this function).
	if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;

	ospfs_direntry_t* entry;
	entry = find_direntry(ospfs_inode(dir->i_ino), dst_dentry->d_name.name, dst_dentry->d_name.len);
	if (entry != NULL)
		return -EEXIST;
	entry = create_blank_direntry(ospfs_inode(dir->i_ino));
	if (IS_ERR(entry))
...
// ospfs_create
//   Linuthe file can't be created;
//               -EIO          on I/O error.
//
//   We have provided strictly less skeleton code for this function than for
//   the others.  Here's a brief outline of what you nx calls this function to create a regular file.
...
//                             exists in the given 'dir';
//               -ENOSPC       if the disk is full & eed to do:
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	ospfs_inode_t *iter; // used as an iterator. 
	...
	dir_entry = create_blank_directory(dir_oi);
	if (IS_ERR(dir_entry))
		return PTR_ERR(dir_entry);
	
	// finds the next empty inode. 
	...
	// got to end of for loop w/o breaking.
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	if (entry_ino < 0)
		return entry_ino; // return the error of the creation. 
	entry_ino = entry->od_ino;

	symlink = (ospfs_symlink_inode_t*)ospfs_inode(entry_ino);

	symlink->oi_size = strlen(symname); // or is it dentry->d_name.len;
